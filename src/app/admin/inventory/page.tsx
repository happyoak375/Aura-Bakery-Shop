            ...
            <div className="flex flex-col md:flex-row gap-4 mb-6">
                <div className="relative flex-1">
                    ...
                    <input
                        ...
                        className="w-full pl-10 pr-4 py-3 rounded-2xl border border-gray-100 focus:outline-none focus:border-black shadow-sm"
                    />
                </div>
                <select
                    value={filterType}
                    onChange={(e) => setFilterType(e.target.value as any)}
                    className="px-4 py-3 rounded-2xl border border-gray-100 focus:outline-none focus:border-black shadow-sm"
                >
                        ...
                        <tbody className="divide-y divide-gray-50">
                            {isLoading ? (
                                ...
                                <tr key={item.id} className="hover:bg-gray-50/50 transition">
                                    ...
                                    <td className="p-4">
                                        <span className={`px-2.5 py-1 text-[10px] uppercase tracking-wider rounded-lg font-bold border ${item.type === 'finished_good' ? 'bg-green-50 text-green-700 border-green-100' :
                                            item.type === 'wip' ? 'bg-blue-50 text-blue-700 border-blue-100' : 'bg-orange-50 text-orange-700 border-orange-100'
                                            }`}>
                                            {item.type === 'finished_good' ? 'Final' : item.type === 'wip' ? 'WIP' : 'Materia Prima'}
                                        </span>
                                    </td>
                                    <td className="p-4">
                                        <span className={`font-bold ${item.currentStock <= item.minStockLevel ? 'text-red-500 bg-red-50 px-2 py-1 rounded-md' : 'text-zinc-900'}`}>
                                            {item.currentStock}
                                        </span>
                                        <span className="text-xs text-zinc-400 ml-1 font-medium">{item.unit}</span>
                                    </td>
                                    <td className="p-4 text-right space-x-1">
                                        <button onClick={() => handleOpenModal(item)} className="p-2 text-zinc-400 hover:text-blue-600 hover:bg-blue-50 rounded-xl transition" title="Editar">
                                            <Edit2 size={18} />
                                        </button>
                                        <button onClick={() => handleDelete(item.id!)} className="p-2 text-zinc-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition" title="Eliminar">
                                            <Trash2 size={18} />
                                        </button>
                                    </td>
                                </tr>
                    ...
                    <div className="bg-white rounded-3xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
                        <div className="p-8">
                            <div className="flex justify-between items-center mb-6">
                                ...
                                <button onClick={() => setIsModalOpen(false)} className="p-2 text-zinc-400 hover:bg-gray-50 hover:text-zinc-900 rounded-full transition"><X /></button>
                            </div>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-6">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <input required type="text" value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} className="w-full p-3 rounded-xl border border-gray-100 focus:border-black focus:outline-none bg-gray-50" />
                                    </div>

                                    <div className="space-y-2">
                                        <label className="text-sm font-bold text-zinc-700">Tipo de Inventario</label>
                                        <select value={formData.type} onChange={e => setFormData({ ...formData, type: e.target.value as any })} className="w-full p-3 rounded-xl border border-gray-100 focus:border-black focus:outline-none bg-gray-50">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <input type="text" value={formData.category} onChange={e => setFormData({ ...formData, category: e.target.value })} className="w-full p-3 rounded-xl border border-gray-100 focus:border-black focus:outline-none bg-gray-50" placeholder="Ej. Clásicos, Café..." />
                                    </div>

                                    <div className="space-y-2">
                                        <label className="text-sm font-bold text-zinc-700">Unidad de Medida</label>
                                        <input required type="text" value={formData.unit} onChange={e => setFormData({ ...formData, unit: e.target.value })} className="w-full p-3 rounded-xl border border-gray-100 focus:border-black focus:outline-none bg-gray-50" />
                                    </div>

                                    <div className="space-y-2">
                                        <label className="text-sm font-bold text-zinc-700">Costo / Precio ($)</label>
                                        <input required type="number" min="0" value={formData.costPerUnit} onChange={e => setFormData({ ...formData, costPerUnit: Number(e.target.value) })} className="w-full p-3 rounded-xl border border-gray-100 focus:border-black focus:outline-none bg-gray-50" />
                                    </div>

                                    <div className="space-y-2">
                                        <label className="text-sm font-bold text-zinc-700">Stock Actual</label>
                                        <input required type="number" min="0" value={formData.currentStock} onChange={e => setFormData({ ...formData, currentStock: Number(e.target.value) })} className="w-full p-3 rounded-xl border border-gray-100 focus:border-black focus:outline-none bg-gray-50" />
                                    </div>

                                    <div className="space-y-2 md:col-span-2">
                                        <label className="text-sm font-bold text-zinc-700">Alerta de Stock Mínimo</label>
                                        <input required type="number" min="0" value={formData.minStockLevel} onChange={e => setFormData({ ...formData, minStockLevel: Number(e.target.value) })} className="w-full p-3 rounded-xl border border-gray-100 focus:border-black focus:outline-none bg-gray-50" />
                                    </div>

                                </div>

                                {/* --- RECETA OPERATIVA (BOM) RESTORED --- */}
                                    ...
                                    <div className="pt-6 border-t border-gray-100 md:col-span-2">
                                        <div className="flex justify-between items-center mb-4">
                                            ...
                                            <button
                                                ...
                                                className="text-[10px] uppercase tracking-wider bg-black text-white px-3 py-2 rounded-lg font-bold hover:bg-zinc-800 transition"
                                            >
                                                + Agregar Insumo
                                            </button>
                                        </div>
                                        ...
                                        {bomItems.length === 0 ? (
                                            <p className="text-sm text-zinc-400 italic bg-gray-50 p-4 rounded-xl border border-gray-100">Este producto no tiene insumos dependientes. Se venderá según su propio stock físico.</p>
                                        ) : (
                                                ...
                                                {bomItems.map((bomItem, index) => (
                                                    <div key={index} className="flex gap-3 items-center">
                                                        <select
                                                            ...
                                                            className="flex-1 p-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-black bg-white shadow-sm"
                                                        >
                                        ...
                                        <div className="flex gap-3">
                                            {['pos', 'web', 'rappi'].map((channel) => (
                                                <button
                                                    ...
                                                    className={`flex-1 py-3 border-2 rounded-xl font-bold uppercase tracking-wider text-xs transition-all ${formData.salesChannels?.includes(channel as any)
                                ...
                                <div className="pt-6 flex justify-end gap-3 border-t border-gray-100">
                                    <button type="button" onClick={() => setIsModalOpen(false)} className="px-6 py-3 font-bold text-zinc-500 hover:bg-gray-50 rounded-xl transition">
                                        Cancelar
                                    </button>
                                    <button type="submit" className="px-8 py-3 font-bold bg-black text-white rounded-xl hover:bg-zinc-800 transition shadow-md active:scale-95">
